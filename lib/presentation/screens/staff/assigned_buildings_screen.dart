...
class AssignedBuildingsScreen extends StatefulWidget {
  ...
  @override
  State<AssignedBuildingsScreen> createState() => _AssignedBuildingsScreenState();
}
    ...
    try {
      ...
      if (staffResponse['success'] == true) {
        ...
        setState(() {
          ...
          _assignedBuildings = List<Map<String, dynamic>>.from(assignedBuildings);
        });
        
        // Check if staff is active and onboarding is completed
        if (staff?['isActive'] != true) {
          AppMessageHandler.showError(context, 'Your staff account is inactive. Please contact admin.');
          return;
        }
        ...
        if (staff?['onboardingIncomplete'] == true) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('⚠️ Your onboarding is not completed. Some features may be limited.'),
        ...
        if (_assignedBuildings.isEmpty) {
          AppMessageHandler.showError(context, 'No buildings assigned. Please contact admin.');
          return;
        }
